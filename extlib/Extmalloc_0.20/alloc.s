...
*
	dc.b	"Ext malloc library - lake - Ver 0.20 for fish or ksh",0
	.even
*
*
...
*  d0	アドレス or -1
*
	movem.l	d1-d5/d7/a0-a4/a6,-(sp)
	bsr	allocate_memory
	movem.l	(sp)+,d1-d5/d7/a0-a4/a6
	rts
*
allocate_memory:
*
...
pool_loop:
	...
	cmp.w	d7,d1
	beq	just_fit_find	* 丁度必要サイズと一致なら直ぐに確保へ
	bcc	pool_loop	* 必要サイズに満たない
	subi.w	#2,d7		* pool のサイズ計算
	cmp.w	d7,d1
	beq	just_fit_find	* 丁度必要サイズ＋２でも直ぐに確保へ
	cmp.w	d7,d5
...
lake_end:			* 最後までサーチ終了
	...
	move.l	d4,a6
	move.l	a6,d7
	bne	larger_find	* 必要最小限があればそれで確保
*
...
*  a6	pool head へのポインタ
*  a2	直前の free pool へのポインタ
*  a3	直後の free pool へのポインタ
*  d7	ワークレジスタ
*
	lea.l	pool_buffer_head(a6),a3
	move.l	a3,d0		* 返り値を予め算出
	addi.w	#2,d1		* 確保サイズは、＋２
	move.w	next_free_offset(a6),d7
	lea	(a6,d7.w),a3	* 次の free pool のポインタ
	move.w	next_pool_offset(a6),d7
	...
	move.l	a3,d7		* 本 free pool と直後の free pool
	sub.l	a6,d7		* の差を計算
