...
.xref word_echo
.xref word_glob
.xref word_refersysroot
.xref word_uncomplete
.xref word_verbose
...
texttop:				*   BIND版  :非BIND版
	...
	dc.l	bssbot-texttop		* 4(texttop):$f4(PDB): BIND版の、切り詰める大きさ
	dc.l	bssbot+STACKSIZE	* 8(texttop):$f8(PDB): スタック・ポインタの初期値
...
i_am_not_root_shell:
		...
		*
		...
		bsr	init_env
		bmi	abort_because_of_insufficient_memory
		*
...
* COMFISH.X
****************
		lea	word_COMFISHOPT,a0
		bsr	fish_getenv
		beq	parse_comfish_opt_done
...
parse_comfish_opt:
		...
		bsr	parse_one_option
		bra	parse_comfish_opt

parse_comfish_opt_done:
		movea.l	PDB_argPtr(a4),a0
		addq.l	#1,a0
skip_command_x_options_1:
		bsr	skip_space
		move.b	(a0),d0
		beq	comfish_ok_3
...
skip_command_x_options_2:
		...
		cmpi.b	#'c',d0
		beq	comfish_ok_1
skip_command_x_options_3:
		move.b	(a0)+,d0
		beq	comfish_ok_3

		cmp.b	#'/',d0
		beq	skip_command_x_options_2

		bsr	isspace2
		bne	skip_command_x_options_3
		bra	skip_command_x_options_1
...
comfish_ok_3:
		moveq	#0,d5
		bra	done_flag_argument_parsing
****************
not_comfish:
		movea.l	PDB_argPtr(a4),a0
		addq.l	#1,a0
...
done_flag_argument_parsing:
	...
	**
		...
		btst.b	#OPTION_t,flags(a5)		*  -t
		bne	set_init_argv

		btst.b	#OPTION_s,flags(a5)		*  -s
		bne	set_init_argv
...
set_init_argv_1:
		bsr	set_argv
	**
	**  interactive_mode を fix する
	**
		tst.l	arg_command(a5)
		bne	reset_interactive_mode

		tst.l	arg_script(a5)
		beq	interactive_mode_ok
reset_interactive_mode:
		sf	interactive_mode(a5)
interactive_mode_ok:
	**
	**  refersysrootの設定
	**
		lea	config_refersysroot,a1
		lea	pathname_buf,a0
		bsr	make_sys_pathname
		bmi	set_refersysroot_ok

		lea	tmpstatbuf,a1
		bsr	stat
		bmi	set_refersysroot_ok

		lea	word_refersysroot,a0
		bsr	set_shellvar_nul
set_refersysroot_ok:
	**
	**  環境変数をシェル変数にインポートする
...
set_initial_vars_done:
	...
	**  スタートアップ
	**
		lea	etc_fishenv,a1				*  $SYSROOT/etc/fishenv
		bsr	run_etc_source_if_any

		btst.b	#OPTION_f,flags(a5)
		bne	start_run

		lea	dot_fishenv,a1				*  ~/.fishenv
		bsr	run_home_dot_source_if_any

		tst.b	interactive_mode(a5)
		beq	start_run

		tst.b	i_am_login_shell(a5)
		beq	source_profile_done

		lea	etc_fprofile,a1				*  $SYSROOT/etc/fprofile
		bsr	run_etc_source_if_any
		lea	dot_fprofile,a1				*  ~/.fprofile
		bsr	run_home_dot_source_if_any
source_profile_done:
		lea	etc_fishrc,a1				*  $SYSROOT/etc/fishrc
		bsr	run_etc_source_if_any
		lea	dot_fishrc,a1				*  ~/.fishrc
		bsr	run_home_dot_source_if_any

		tst.b	i_am_login_shell(a5)
		beq	not_source_login

		lea	etc_flogin,a1				*  $SYSROOT/flogin
		bsr	run_etc_source_if_any
		lea	dot_login,a1				*  ~/.login
		bsr	run_home_dot_source_if_any
		bra	do_source_fishdirs

not_source_login:
		btst.b	#OPTION_d,flags(a5)
		beq	source_fishdirs_done
do_source_fishdirs:
		lea	dot_fishdirs,a1				*  ~/.fishdirs
		moveq	#(1<<SOURCE_FLAGBIT_NOSUBST),d7
		bsr	run_home_dot_source_if_any_1
source_fishdirs_done:
startup_done:
		tst.b	interrupted(a5)
		bne	start_run

		btst.b	#OPTION_h,flags(a5)
		...
		lea	load_history_done(pc),a0
		move.l	a0,mainjmp(a5)
		lea	dot_history,a1				*  ~/.history
		moveq	#((1<<SOURCE_FLAGBIT_ONLYLOAD)|(1<<SOURCE_FLAGBIT_NOCOMMENT)|(1<<SOURCE_FLAGBIT_NOCONTLINE)),d7
		bsr	run_home_dot_source_if_any_1
load_history_done:
start_run:
		lea	abort_shell_status(pc),a0
		move.l	a0,mainjmp(a5)
	**
	**  -v と -x を処理する
	**
		btst.b	#OPTION_v,flags(a5)		*  -v
		beq	set_verbose_done

		bsr	set_verbose
set_verbose_done:
		btst.b	#OPTION_x,flags(a5)		*  -x
		beq	start_run_1
...
do_argument:
		tst.b	breaked(a5)
		bne	abort_shell_status

		moveq	#0,d0
		move.l	arg_command(a5),d1
		bmi	exit_shell_d0
...
parse_one_option:
		...
		cmp.b	#'k',d0			*  -k : 作業ディレクトリを元に戻さずに終了する
		beq	flag_k_found
		...
		bsr	issjis
		bne	parse_one_option_done

		tst.b	(a0)
		beq	parse_one_option_done

		addq.l	#1,a0
parse_one_option_done:
		rts
...
init_bss:
		...
		clr.l	shellvar_top(a5)		*  シェル変数リストを初期化
		clr.l	alias_top(a5)			*  別名リストを初期化
		clr.l	completion_top(a5)		*  補完式リストを初期化
...
free_current_argbuf_return:
		...
		rts
*****************************************************************
.xdef free_tmpgetlinebuf

free_all_tmp_and_misc_buffers:
		bsr	free_all_tmp
...
break_shell:
		...
		*
		move.l	d0,-(a7)
		bsr	free_all_tmp_and_misc_buffers
		move.l	(a7)+,d0
free_argbuf_loop:
...
logout:
		...
		move.l	a7,stackp(a5)
		st	doing_logout
		lea	etc_flogout,a1			*  /etc/flogout
		bsr	run_etc_source_if_any
		lea	dot_logout,a1			*  ~/.logout
		bsr	run_home_dot_source_if_any
logout_terminated:
		sf	doing_logout
		bsr	reset_delete_io
source_logout_done:
		bsr	disable_break
		move.w	d0,-(a7)			*  現在のブレーク・フラグをスタックに保存
		move.l	a7,stackp(a5)
...
*      D1.B   bit0:A0が単語並びならば1; bit1:bg実行ならば1
*      D2.B   -n フラグ
*
* RETURN
*      D0.L   ステータス
...
run_command_in_bg:
		moveq	#3,d1
		bra	run_command_in_subshell_1

run_command_in_subshell:
		moveq	#1,d1
run_command_in_subshell_1:
...
load_source_fail:
		...
		bra	print_shell_error
****************************************************************
* make_home_dot_filename
*
* CALL
*      A1     ファイル名
*
* RETURN
*      pathname_buf   パス名が格納される
*      A0     パス名（pathname_buf の先頭）
*      A3     パス名のファイル名部の先頭
*      CCR    MI ならエラー
*             o シェル変数 home が定義されていない
*             o シェル変数 home の単語数が 0個である
*             o シェル変数 home の 1番目の単語が空である
*             o パス名が長過ぎる
****************************************************************
make_home_dot_filename:
		movem.l	d0/a1-a2,-(a7)
		...
		lea	word_home,a0
		bsr	get_shellvar
		beq	make_home_dot_filename_fail
		...
		lea	pathname_buf,a0
		bsr	cat_pathname
make_home_dot_filename_return:
		movem.l	(a7)+,d0/a1-a2
		rts

make_home_dot_filename_fail:
		moveq	#-1,d0
		bra	make_home_dot_filename_return
*****************************************************************
...
run_etc_source_if_any:
		lea	pathname_buf,a0
		bsr	make_sys_pathname
		bmi	run_source_return

		clr.b	d7
		bra	run_source_if_any

run_home_dot_source_if_any:
		clr.b	d7
run_home_dot_source_if_any_1:
		bsr	make_home_dot_filename
		bmi	run_source_return
...
check_quotes_break:
		move.b	d3,d0
		bne	check_quotes_unmatched
check_parens_and_quotes_continue:
		dbra	d1,check_parens_and_quotes_loop

		tst.w	d2
		bne	test_line_unmatched_paren

		movem.l	(a7)+,d0-d3/a0
		rts

test_line_unmatched_paren:
		lea	msg_unmatched_parens,a0
		bra	print_shell_error

check_quotes_unmatched:
		bsr	reset_delete_io
		bsr	unmatched
		bra	shell_error
*****************************************************************
...
*
* RETURN
*      D0.L    -1:EOF，-2: 入力エラー+EOF，otherwise 0
*      CCR     TST.L D0
*      その他  破壊
*****************************************************************
do_line_getline:
...
do_line_substhist:
		...
		**
		...
		movea.l	d0,a2
		btst.b	#SOURCE_FLAGBIT_ONLYLOAD,SOURCE_FLAGS(a2)
		bne	do_not_substhist

		btst.b	#SOURCE_FLAGBIT_NOSUBST,SOURCE_FLAGS(a2)
		bne	do_not_substhist
do_line_substhist_1:
		tst.b	funcdef_status(a5)
		bne	do_not_substhist
...
find_words_1:
		...
		**
		...
		movea.l	d1,a1
		btst.b	#SOURCE_FLAGBIT_ONLYLOAD,SOURCE_FLAGS(a1)
		beq	skip_enter_history
...
get_redirect_filename:
		...
		bsr	subst_var
		movem.l	(a7)+,a0-a1
		beq	redirect_name_error_2
		bmi	redirect_name_error_2
		...
		cmp.l	#-5,d0
		bne	redirect_name_error_2

		moveq	#0,d0
redirect_name_error_2:
		cmp.l	#-4,d0
		beq	shell_error
...
redirect_out_device_check:
		bsr	ischardev			* そいつがキャラクタ・デバイスかどうかを
		move.b	d0,d1				*   D1にセット
		moveq	#1,d0
...
*      simple_args
*      argc
*      D1.B   bit 0 : 消費時間報告  0:$timeに従う  1:常に行う
*             bit 1 : 組み込みコマンドエラー  0:アボートする  1:アボートしない
*             bit 2 : 外部コマンドに対する引数エンコード  0:する  1:しない
*             bit 3-4 : 外部コマンドに対する引数が255バイトを超えた場合
*                       bit4 bit3
*                         0    0    $@hugearg に従う
*                         0    1    force
*                         1    0    indirect
*                         1    1    error
*             bit 5 : 外部コマンドによる作業ディレクトリ変更  0:許さない  1:許す
*             bit 6 : 外部コマンドによる環境変数変更  0:許さない  1:許す
*             bit 7 : 1:組み込みコマンドと関数を除外して外部コマンドのみ検索する
*      D2.B   非0:再帰である..変数展開をしない，入出力をリセットしない
*             負ならば，さらに一切の置換・展開をしない
*      A2     D1.B bit4==1,bit3==0 のとき, indirect flag文字列
*
* RETURN
*      全て   破壊
*****************************************************************
...
status = -4
argp = status-4
timer_exec_start_low = argp-4
timer_exec_start_high = timer_exec_start_low-4
DoSimpleCommand_indirect_flag = timer_exec_start_high-4
DoSimpleCommand_flags = DoSimpleCommand_indirect_flag-1
recursed = DoSimpleCommand_flags-1
arg_is_huge = recursed-1
status_ok = arg_is_huge-1
timer_ok = status_ok-1
pad = timer_ok-1
...
DoSimpleCommand:
		...
		sf	status_ok(a6)
		sf	timer_ok(a6)
		move.b	d1,DoSimpleCommand_flags(a6)
		move.b	d2,recursed(a6)
		move.l	a2,DoSimpleCommand_indirect_flag(a6)
		move.w	argc(a5),d0
		beq	simple_command_done_0

		tst.b	d2
		bpl	DoSimpleCommand_1
****************
		lea	simple_args(a5),a0
		bsr	strlen
		cmp.l	#MAXPATH,d0
...
command_name_ok:
		btst.b	#FLAG_NO_BUILTIN,DoSimpleCommand_flags(a6)
		bne	not_function
...
function_paren_ok:
		...
		tst.b	recursed(a6)
		bmi	simple_run_function_arg_ok
...
not_function:
		...
		moveq	#0,d0
		btst.b	#FLAG_NO_BUILTIN,DoSimpleCommand_flags(a6)
		beq	do_simple_command_search_command

		moveq	#1,d0				*  ~~ は検索しない
do_simple_command_search_command:
	...
	*
		...
		tst.b	recursed(a6)
		bpl	run_builtin_flip_flop

		tst.l	undup_input(a5)
		bpl	run_simple_command_in_subshell
run_builtin_flip_flop:
		not.b	pipe_flip_flop(a5)
builtin_io_ok:
		btst.b	#3,8(a2)
		beq	builtin_no_sub
...
run_builtin:
		...
		*  組み込みコマンドのエラー
		btst.b	#FLAG_NO_ABORT,DoSimpleCommand_flags(a6)
		beq	shell_error			*  アボート

		moveq	#1,d0
		bra	simple_command_done_x		*  status に 1 をセットして正常終了
...
do_script_with_explicit_shell:
		...
		movea.l	a3,a0
		move.l	d3,d0
		bsr	EncodeHUPAIRx			*  コマンドラインにエンコードする
		bmi	simple_command_too_long_line
...
****************
		...
		*
		...
		move.b	DoSimpleCommand_flags(a6),d0
		lsr.b	#3,d0
		and.b	#3,d0
		beq	hugearg_var			*  $@hugearg に従う

		subq.b	#1,d0
		beq	do_exec				*  force

		subq.b	#1,d0
		bne	hugearg_abort			*  error
		*  indirect
		movea.l	DoSimpleCommand_indirect_flag(a6),a1
		bra	hugearg_indirect_check_flag
...
hugearg_indirect:
		cmp.w	#2,d2
		blo	hugearg_indirect_default_flag

		bsr	strfor1
		movea.l	a0,a1
hugearg_indirect_check_flag:
		tst.b	(a1)
		bne	hugearg_indirect_flag_ok
hugearg_indirect_default_flag:
...
loadprg_stop:
		...
		st	in_fish
		move.l	d0,-(a7)
		btst	#FLAG_CDCMD,DoSimpleCommand_flags(a6)
		sne	d0
		bsr	resume_cwd
		btst	#FLAG_ENVCMD,DoSimpleCommand_flags(a6)
		sne	d0
		bsr	resume_env
		move.l	(a7)+,d0
		bmi	exec_failure
...
not_reset_io:
		...
		btst.b	#FLAG_TIME_ALWAYS,DoSimpleCommand_flags(a6)
...
****************************************************************
EncodeHUPAIRx:
		btst.b	#FLAG_RAWARG,DoSimpleCommand_flags(a6)
		beq	EncodeHUPAIR
...
****************************************************************
* init_env - fish の環境変数を初期化する
*
* CALL
*      A0     取り込む環境エリアの先頭アドレス（DOS形式）
*
* RETURN
*      D0.L   メモリが足りなければ -1，正常に完了しらなら 0
*      CCR    TST.L D0
****************************************************************
init_env:
		movem.l	d1/a0-a1,-(a7)
...
built_user_env:
		...
		moveq	#0,d2
built_user_env_calc_loop:
		cmpa.l	#0,a1
		beq	built_user_env_calc_done

		movea.l	a1,a0
		bsr	varsize
		add.l	d0,d2
		movea.l	var_next(a1),a1
		bra	built_user_env_calc_loop

built_user_env_calc_done:
		lea	word_envmargin,a0
		bsr	get_shellvar
		beq	built_user_env_margin_ok

		bsr	atou
		bmi	built_user_env_margin_ok
		bne	built_user_env_fail

		add.l	d1,d2				*  + margin
built_user_env_margin_ok:
		addq.l	#4+1+1,d2			*  + size field + tail null + pad
		bclr	#0,d2				*  D2 : 環境エリアのサイズ
		move.l	d2,d0
		bsr	xmalloc
		beq	built_user_env_fail
...
built_user_env_loop:
		...
		bra	built_user_env_loop

built_user_env_done:
		clr.b	(a0)
		moveq	#0,d0
built_user_env_return:
		movem.l	(a7)+,d1-d2/a0-a2
		rts

built_user_env_fail:
		moveq	#-1,d0
		bra	built_user_env_return
****************************************************************
...
remember_misc_environments:
		rts
****************************************************************
* resume_cwd - 自プロセスのcwdを復旧する
*
* CALL
*      D0.B   0 なら、実行したプログラムが cdcmds に設定されてい
*             るコマンド以外なら復旧する
*             正なら必ず復旧する
*             負なら決して復旧しない
*
...
resume_cwd:
		...
		lea	cwd(a5),a1
		bsr	strcmp
		beq	resume_cwd_return		*  作業ディレクトリは変わっていない

		tst.b	d1
		bmi	resume_cwd_change
		bne	do_resume_cwd
		...
		movem.l	(a7)+,d0/a0
		bsr	wordlistpcmp
		beq	resume_cwd_change		*  現状の作業ディレクトリを受け入れる
do_resume_cwd:
		*  作業ディレクトリを元に戻す
		lea	cwd(a5),a0
		bsr	chdir
...
resume_cwd_return:
		...
		rts
****************************************************************
* resume_env - 実行したプログラムが envcmds に設定されているコマ
*              ンドなら自プロセスの環境変数を変更する
*
* CALL
*      D0.B   0 なら、実行したプログラムが envcmds に設定されてい
*             るコマンドなら変更する
*             負なら必ず変更する
*
* RETURN
...
resume_env:
		...
		bne	free_userenv
resume_env_change:
		movea.l	user_command_env(a5),a0
		bsr	init_env
free_userenv:
		lea	user_command_env(a5),a0
		bsr	xfreep
...
report_time_second:
		...
		moveq	#'.',d6
		bsr	report_time_print_field
		bsr	report_time_printfi
		moveq	#CR,d0
		bsr	report_time_putc
		moveq	#LF,d0
		bsr	report_time_putc
...
report_time_putc_1:
		...
		rts
****************************************************************
builtin_dir_match_sub:
		move.l	a1,-(a7)
		movea.l	a0,a1
		lea	str_builtin_dir,a0
		bsr	strlen
		move.l	d0,d1
		bsr	memcmp
		lea	(a1,d1.l),a0
...
builtin_dir_match:
		movem.l	d1/a0,-(a7)
		bsr	builtin_dir_match_sub
		bne	builtin_dir_match_false
		...
		bsr	find_slashes
builtin_dir_match_1:
		beq	builtin_dir_match_return
builtin_dir_match_false:
		moveq	#0,d1
builtin_dir_match_return:
		move.l	d1,d0
		movem.l	(a7)+,d1/a0
		rts
****************************************************************
...
*
*      拡張子が .R .X .BAT でなく，x bit が立っていないファイルは
*      実行不可．
*****************************************************************
statbuf = -STATBUFSIZE
...
find_command_file_done:
		...
		cmp.l	#6,d1
		blo	find_command_file_done_1

		moveq	#0,d1
find_command_file_done_1:
		movea.l	a3,a0
		bsr	headtail			*  A1 : tail point
		tst.b	d3				*  拡張子を'.*'として検索したならば
		beq	find_command_file_done_2	*  realname には間違いなく本当の名前が格納されている

		bsr	lgetmode
		btst	#MODEBIT_LNK,d0			*  ファイルがシンボリック・リンクでないならば
		beq	find_command_file_done_2	*  realname には間違いなく本当の名前が格納されている

		*  シンボリック・リンクであるから、realname は本当の名前ではない
		*  ワイルドカードで検索して本当の名前を得る
		movea.l	a1,a2				*  A2 : tail point
		tst.b	(a2)				*  （有り得ないが）ファイル名が空なら
		beq	find_command_file_return	*  そのまま帰る

		lea	fatchkbuf1,a1
		bsr	fatchk
		bmi	find_command_file_miss		*  ??? 存在する筈なのに存在しない ???

		move.b	#'?',(a2)			*  ファイル名の先頭を'?'として検索する
		move.w	#MODEVAL_ALL,-(a7)
		move.l	a0,-(a7)
		pea	statbuf(a6)
		DOS	_FILES
		lea	10(a7),a7
find_command_realname_loop:
		bmi	find_command_file_miss		*  ??? 存在する筈なのに存在しない ???
		...
		lea	fatchkbuf2,a1
		bsr	fatchk
		bmi	find_command_realname_next

		lea	fatchkbuf1,a3
		cmpm.w	(a1)+,(a3)+
		bne	find_command_realname_next

		cmpm.l	(a1)+,(a3)+
		beq	find_command_file_return
find_command_realname_next:
		pea	statbuf(a6)
		DOS	_NFILES
		addq.l	#4,a7
		bra	find_command_realname_loop

find_command_file_done_2:
		movea.l	a1,a0
		lea	realname(a6),a1
		bsr	strcpy
...
exp_command_name1 = -auto_pathname
exp_command_name2 = exp_command_name1-auto_pathname
...
search_command:
		...
		bne	search_command_not_found

		movea.l	a0,a1
		lea	exp_command_name1(a6),a0
		moveq	#MAXPATH,d0
		bsr	fair_pathname
		bcs	search_command_not_found
...
search_command_in_pathlist_hash_done:
		...
		lea	exp_command_name2(a6),a1	*  A1 : buffer
search_command_in_pathlist_loop:
		...
		*  それでも，相対パスである場合には探す
		...
		bsr	is_builtin_dir
		bne	search_command_in_pathlist_next	*  仮想ディレクトリである
...
search_command_tryone:
		...
		bne	search_command_in_pathlist_next	*  いるならば無効
...
fish_copyright:	dc.b	'Copyright(C)1991-93 by Itagaki Fumihiko',0
...
word_getopt:		dc.b	'getopt',0
word_goto:		dc.b	'goto',0
word_hashstat:		dc.b	'hashstat',0
word_builtin_noabort:	dc.b	'~~/'
word_noabort:		dc.b	'noabort',0
word_nop:		dc.b	'nop',0
...
str_COM:		dc.b	'COM'
word_COMFISHOPT:	dc.b	'COMFISHOPT',0
init_batshell:		dc.b	'COMMAND',0
init_shell:		dc.b	'fish',0,'-f',0
config_refersysroot:	dc.b	'/etc/config/refersysroot',0
etc_fishenv:		dc.b	'/etc/fishenv',0
etc_fprofile:		dc.b	'/etc/fprofile',0
etc_fishrc:		dc.b	'/etc/fishrc',0
etc_flogin:		dc.b	'/etc/flogin',0
etc_flogout:		dc.b	'/etc/flogout',0
dot_fishenv:		dc.b	'.fishenv',0
dot_fprofile:		dc.b	'.fprofile',0
dot_fishrc:		dc.b	'.fishrc',0
dot_login:		dc.b	'.login',0
dot_logout:		dc.b	'.'
word_logout:		dc.b	'logout',0
dot_fishdirs:		dc.b	'.fishdirs',0
word_upper_columns:	dc.b	'COLUMNS',0
...
word_cwdcmd:		dc.b	'cwd'
word_cmd:		dc.b	'cmd',0
word_envcmds:		dc.b	'envcmds',0
...
subst_tmpword1:			ds.b	MAXWORDLEN+1
subst_tmpword2:			ds.b	MAXWORDLEN+1
...
doscall_pathname:		ds.b	MAXPATH+1
.even
...
tmppwbuf:			ds.b	PW_SIZE
outbuf:				ds.b	OUTBUF_SIZE
*****************************************************************
.offset 0

**  シェル／サブシェル毎のデータ
**  メインシェルで初期化され，サブシェルではカレントシェルから引き継ぐ
**  サブシェルも変更する
...
old_cwd:		ds.b	MAXPATH+1
cwd:			ds.b	MAXPATH+1

xbss_bottom:
XBSS_SIZE		equ	xbss_bottom-xbss_top

**  シェル／サブシェル毎のデータ
**  シェル／サブシェル毎にinitializeする
...
**  シェル／サブシェル／source毎のデータ
**  シェル／サブシェル／source毎にinitializeする
